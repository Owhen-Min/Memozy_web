...
function Modal({ isOpen, onClose, data, isLoading }: ModalProps) {
  ...
  return (
    <>
      {/* WrongAnswerModal*/}
      <div
        className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center font-pre-regular z-40"
        onClick={handleOutsideClick}
      >
        <div
              ...
              >
                ...
                <div className="flex flex-col md:flex-row md:justify-between mt-1 md:mt-0">
                  <p className="text-12 md:text-14 text-gray-600">
                    전체 퀴즈 개수: {history.allCount} <span className="mx-1"></span> 틀린 퀴즈
                    개수:
                    {history.failCount}
                  </p>
      ...
      {isSummaryModalOpen && selectedSummary && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-[9999]">
          <div className="mt-14 w-full h-full flex items-center justify-center">
            <NoteModal
